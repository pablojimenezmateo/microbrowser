...
Result Interpreter::EvaluateStatement(const Node& node, Environment& scope) {
  ...
  switch (node.kind) {
    ...
    case NodeKind::VariableDeclaration: {
      ...
      // A `var`'s binding was made when the function was entered, in the
      // function's scope, wherever in the body this line sits. So this line
      // *assigns* -- declaring here would make a second binding in whatever
      // block it happens to be written in, which is what gave `var` block
      // scope: `{ var n = 1 } return n` wrote the 1 into the block and read
      // undefined out of the function.
      ...
      for (const NodePtr& declarator : node.children) {
        ...
        if (declarator->Child(1) != nullptr) {
          ...
          // `var n;` on its own does nothing to a binding that already exists,
          // so `var n = 1; var n;` keeps the 1.
          continue;
        }
        const Result bound = BindPattern(*target, value, scope, !is_var, is_const);
        if (bound.IsAbrupt()) {
          return bound;
        }
      }
