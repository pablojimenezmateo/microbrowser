...
class Reflector {
 ...
 private:
  ...
  // HTMLHyperlinkElementUtils on `<a>` and `<area>`, defined in UrlObject.cpp over the same
  // parser every fetch goes through -- declared here rather than on `DomBindings` because that
  // header is at its cap and this is a member that was never state. Friendship is what makes
  // that possible, and the reason it is the *reflector* that adopted it is that the
  // decomposition attributes are installed in the same pass as every other reflected accessor.
