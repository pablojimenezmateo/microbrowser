#include "util/AesCtr.h"

#include <array>
#include <cstring>

namespace microbrowser::util {

namespace {

constexpr std::uint8_t kSbox[256] = {
    };

constexpr std::uint8_t kRcon[10] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};

std::uint8_t Xtime(std::uint8_t x) {
  return static_cast<std::uint8_t>((x << 1) ^ ((x & 0x80) ? 0x1b : 0x00));
}

std::uint32_t SubWord(std::uint32_t word) {
  return (static_cast<std::uint32_t>(kSbox[(word >> 24) & 0xff]) << 24) |
         (static_cast<std::uint32_t>(kSbox[(word >> 16) & 0xff]) << 16) |
         (static_cast<std::uint32_t>(kSbox[(word >> 8) & 0xff]) << 8) |
         static_cast<std::uint32_t>(kSbox[word & 0xff]);
}

std::uint32_t RotWord(std::uint32_t word) {
  return (word << 8) | (word >> 24);
}

void KeyExpansion128(const std::uint8_t* key, std::array<std::uint8_t, 176>& round_keys) {
  std::uint32_t w[44];
  for (int i = 0; i < 4; ++i) {
    w[i] = (static_cast<std::uint32_t>(key[i * 4]) << 24) |
           (static_cast<std::uint32_t>(key[i * 4 + 1]) << 16) |
           (static_cast<std::uint32_t>(key[i * 4 + 2]) << 8) |
           static_cast<std::uint32_t>(key[i * 4 + 3]);
  }
  for (int i = 4; i < 44; ++i) {
    std::uint32_t temp = w[i - 1];
    if (i % 4 == 0) {
      temp = SubWord(RotWord(temp)) ^ static_cast<std::uint32_t>(kRcon[i / 4 - 1] << 24);
    }
    w[i] = w[i - 4] ^ temp;
  }
  for (int i = 0; i < 44; ++i) {
    round_keys[static_cast<std::size_t>(i * 4)] = static_cast<std::uint8_t>((w[i] >> 24) & 0xff);
    round_keys[static_cast<std::size_t>(i * 4 + 1)] = static_cast<std::uint8_t>((w[i] >> 16) & 0xff);
    round_keys[static_cast<std::size_t>(i * 4 + 2)] = static_cast<std::uint8_t>((w[i] >> 8) & 0xff);
    round_keys[static_cast<std::size_t>(i * 4 + 3)] = static_cast<std::uint8_t>(w[i] & 0xff);
  }
}

void AddRoundKey(std::uint8_t state[16], const std::uint8_t* round_key) {
  for (int i = 0; i < 16; ++i) {
    state[i] ^= round_key[i];
  }
}

void SubBytes(std::uint8_t state[16]) {
  for (int i = 0; i < 16; ++i) {
    state[i] = kSbox[state[i]];
  }
}

void ShiftRows(std::uint8_t state[16]) {
  const std::uint8_t tmp[16] = {
      state[0],  state[5],  state[10], state[15], state[4],  state[9],  state[14], state[3],
      state[8],  state[13], state[2],  state[7],  state[12], state[1],  state[6],  state[11]};
  std::memcpy(state, tmp, 16);
}

void MixColumns(std::uint8_t state[16]) {
  for (int col = 0; col < 4; ++col) {
    const int i = col * 4;
    const std::uint8_t a0 = state[i];
    const std::uint8_t a1 = state[i + 1];
    const std::uint8_t a2 = state[i + 2];
    const std::uint8_t a3 = state[i + 3];
    const std::uint8_t t = static_cast<std::uint8_t>(a0 ^ a1 ^ a2 ^ a3);
    state[i] = static_cast<std::uint8_t>(a0 ^ t ^ Xtime(static_cast<std::uint8_t>(a0 ^ a1)));
    state[i + 1] = static_cast<std::uint8_t>(a1 ^ t ^ Xtime(static_cast<std::uint8_t>(a1 ^ a2)));
    state[i + 2] = static_cast<std::uint8_t>(a2 ^ t ^ Xtime(static_cast<std::uint8_t>(a2 ^ a3)));
    state[i + 3] = static_cast<std::uint8_t>(a3 ^ t ^ Xtime(static_cast<std::uint8_t>(a3 ^ a0)));
  }
}

void Aes128EncryptBlock(const std::uint8_t* key, const std::uint8_t block[16], std::uint8_t out[16]) {
  std::array<std::uint8_t, 176> round_keys{};
  KeyExpansion128(key, round_keys);

  std::uint8_t state[16];
  std::memcpy(state, block, 16);
  AddRoundKey(state, round_keys.data());

  for (int round = 1; round < 10; ++round) {
    SubBytes(state);
    ShiftRows(state);
    MixColumns(state);
    AddRoundKey(state, round_keys.data() + round * 16);
  }
  SubBytes(state);
  ShiftRows(state);
  AddRoundKey(state, round_keys.data() + 160);

  std::memcpy(out, state, 16);
}

void IncrementCounter(std::uint8_t counter[16]) {
  for (int i = 15; i >= 0; --i) {
    if (++counter[i] != 0) {
      break;
    }
  }
}

}  // namespace

bool Aes128CtrXor(std::span<const std::uint8_t, 16> key, std::span<const std::uint8_t, 16> counter,
                  std::span<const std::uint8_t> input, std::span<std::uint8_t> output) {
  if (output.size() != input.size()) {
    return false;
  }

  std::uint8_t ctr[16];
  std::memcpy(ctr, counter.data(), 16);

  std::size_t offset = 0;
  while (offset < input.size()) {
    std::uint8_t keystream[16];
    Aes128EncryptBlock(key.data(), ctr, keystream);

    const std::size_t chunk = std::min<std::size_t>(16, input.size() - offset);
    for (std::size_t i = 0; i < chunk; ++i) {
      output[offset + i] = static_cast<std::uint8_t>(input[offset + i] ^ keystream[i]);
    }
    offset += chunk;
    if (offset < input.size()) {
      IncrementCounter(ctr);
    }
  }
  return true;
}

}  // namespace microbrowser::util
