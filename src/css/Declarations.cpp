...
// `*-content` spellings of the CSS Box Alignment module are accepted alongside
// the flexbox ones: a page writes `start` as often as `flex-start`.
...
void ApplyDeclaration(const Declaration& declaration, const ComputedStyle& parent,
  ...
  if (property == "flex-flow") {
    ...
    for (const std::string_view part : SplitWords(value)) {
      ApplyDeclaration(Declaration{"flex-direction", std::string(part), false}, parent, style);
      ApplyDeclaration(Declaration{"flex-wrap", std::string(part), false}, parent, style);
    }
    return;
  }
  ...
  if (property == "flex-grow" || property == "flex-shrink") {
    ...
    if (!number.has_value() || *number < 0.0) {
      return;  // negative is invalid, and an invalid declaration is dropped
    }
  ...
  if (property == "flex") {
    ...
    for (const std::string_view part : parts) {
      ...
      if (numeric && numbers < 2) {
        ApplyDeclaration(
            Declaration{numbers == 0 ? "flex-grow" : "flex-shrink", std::string(part), false},
            parent, style);
        ++numbers;
        continue;
      }
      ApplyDeclaration(Declaration{"flex-basis", std::string(part), false}, parent, style);
      saw_basis = true;
    }
