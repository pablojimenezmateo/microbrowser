#!/usr/bin/env bash
# Drive one unit of work at a time, each in a brand-new agent process.
#
# Two ledgers, two commands, one loop:
#   /next-session   -> docs/roadmap-sessions.json, sequenced by which page it unblocks
#   /next-wpt-task  -> docs/wpt-tasks.json, ranked by test files Firefox passes and we do not
#
...
# from the only things that survive — the git log, docs/session-log.md, and whichever ledger the
# command reads. If a session cannot hand off through those three files, it has not finished, and
# that is the constraint that keeps them honest.
#
#   tools/agent-loop.sh                 # one session, interactive, you approve each tool call
#   tools/agent-loop.sh -n 5            # five sessions back to back
#   tools/agent-loop.sh -c /next-wpt-task -n 5   # ...working the WPT plan instead
...
case "$command_name" in
  /next-session)  ledger="docs/roadmap-sessions.json" ;;
  /next-wpt-task) ledger="docs/wpt-tasks.json" ;;
  *) echo "unknown command: $command_name (expected /next-session or /next-wpt-task)" >&2; exit 2 ;;
esac
...
echo "repo:     $repo_root"
echo "branch:   $branch"
echo "command:  $command_name  (ledger: $ledger)"
...
for ((i = 1; i <= sessions; i++)); do
  ...
  if ! timeout "$timeout_s" claude "${claude_args[@]}" "$command_name" >"$transcript" 2>&1; then
    status=$?
