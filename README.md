...
| M8 | JavaScript: lexer, parser, bytecode VM, GC, builtins | **in progress** — lexer, parser, a bytecode compiler and machine, mark-sweep GC that runs *during* evaluation, classes, regular expressions, Promises, `async`/`await` and a standard library done; generators, `Proxy` and modules not. `await` is what the machine was built for: a frame is a record, so a waiting call is filed whole and put back by a microtask, which is not a thing C++ stack frames can be. Generators are the same machinery. See ADR 0007. |
