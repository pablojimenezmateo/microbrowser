...
| M8 | JavaScript: lexer, parser, bytecode VM, GC, builtins | **in progress** — lexer, parser, a bytecode compiler and machine, mark-sweep GC that runs *during* evaluation, classes, regular expressions, `Proxy`, Promises, `async`/`await`, generators, async generators, `for await` and a standard library done; modules not. Suspending a call is what the machine was built for: a frame is a record, so a waiting one is filed whole and put back later, which is not a thing C++ stack frames can be. `await` and `yield` are that one mechanism with different triggers. There is now no syntax the parser accepts that the compiler hands back to the tree-walker; what is left there is the differential engine. See ADR 0007. |
